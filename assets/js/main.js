...
(function() {
  ...
  function aosInit() {
    ...
    // ensure AOS recalculates positions after any late layout shifts
    setTimeout(() => {
  ...
  document.querySelectorAll('.isotope-layout').forEach(function(isotopeItem) {
    ...
    imagesLoaded(isotopeItem.querySelector('.isotope-container'), function() {
      ...
      // refresh AOS after isotope finishes layout (images may have changed heights)
      if (typeof AOS !== 'undefined' && AOS.refresh) AOS.refresh();
    });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.skill-box').forEach(box => {
      const img = box.querySelector('.skill-svg');
      if (!img) return;
      img.addEventListener('error', () => {
        img.style.display = 'none';
        const label = box.querySelector('.skill-label')?.textContent || '';
        const initials = label.split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase();
        const el = document.createElement('div');
        el.className = 'skill-fallback';
        ...
        el.style.background = 'linear-gradient(135deg,#6dd5ed,#2193b0)';
        el.style.color = '#fff';
        el.style.fontWeight = '700';
        el.style.marginBottom = '8px';
        box.insertBefore(el, box.firstChild);
      });
      // trigger load check for cached errors
